...
int main(void) {
	...
	cli_inicializarArrayPunteros(punterosClientes,CANTIDAD_CLIENTES);
	...
	do
	{
							...
							"\nError opcion invalida",1,11,2) )
		{
			...
			case 1:
				...
				auxiliarIndice = cli_getEmptyIndexConArrayPunteros(punterosClientes,CANTIDAD_CLIENTES);
				...
				if(auxiliarIndice >= 0)
				{
					...
					if(cli_altaArrayConPunteros(punterosClientes,CANTIDAD_CLIENTES,auxiliarIndice,&idClientes) == 0)
					{
						printf("\nCarga realizada con exito, el ID del cliente es: %d\n", idClientes-1);
					}
			...
			case 6:
				...
				break;
			case 7:
				// iterar array de punteros
				// imprimir datos de cliente. ojo acceder con ->
				// cada item del array es un puntero a cliente
