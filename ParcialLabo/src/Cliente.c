...
int cli_inicializarArrayPunteros(Cliente* array[],int limite)
//int cli_inicializarArrayPunteros(Cliente** array,int limite)
{
	int respuesta = -1;
	int i;
...
int cli_altaArrayConPunteros(Cliente* array[],int limite, int indice, int* idCliente)
{
	...
	if(array != NULL && limite > 0 && indice < limite && indice >= 0 && idCliente != NULL)
	{
		...
		{
			// cargo los datos en bufferAux
			bufferAux.idCliente = *idCliente;
			//bufferAux.isEmpty = 0; // NO!
			//array[indice] = bufferAux; // NO!, cambia

			pC = cli_new();
			if(pC!=NULL)
			{
				//bufferAux aca estan los datos
				//el espacio de memoria esta apuntado por pC
				*pC = bufferAux; // copiar los campos de bufferAux en el area dinamica

				array[indice] = pC; // copio direccion en array de punteros
